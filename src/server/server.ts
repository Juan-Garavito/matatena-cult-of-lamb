...
import type{ Player, GameWrapper, JoinGameRequest } from './types/game.types.js';
import { JoinGameRequestSchema, PlayRequestSchema } from './types/game.schema.js';
...
app.post('/join-game', async (req, res) => {
  const {success, data} = await JoinGameRequestSchema.safeParseAsync(req.body as JoinGameRequest);
  ...
  const newPlayer : Player = createPlayer(data.playerName)
  ...
  res.json({ success: true, player: {id: newPlayer.id, name: newPlayer.name} });
});
...
io.on('connection', (socket) => {
  ...
  socket.on('play/'+idGame, async (data) => {
    console.log('Jugada: id_game='+data.id_game+', column='+data.column+', dice='+data.dice);
    const {success, data: playRequest} = await PlayRequestSchema.safeParseAsync(data);
